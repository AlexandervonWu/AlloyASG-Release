...
public class ComplexNumber
{
	...
	*/
	...
	/**
	 * Used in <code>format(int)</code> to format the complex number as R.cis(theta), where theta is arg(z)/PI
	 */
	public static final int RCISPI = 2;
	/**
	...
	*/
	...
	/**
	 * Constructs a new <code>ComplexNumber</code> object with only real part.
	 * @param real the real part, Re(z), of the complex number
	 */
	...
	*/
	...
	/**
	 * Multiplies the current <code>ComplexNumber</code> by a scalar.
	 * @param scalar the scalar to be multiplied to the current complex number
	 * 
	 */
	public void multiply(double scalar)
	{
		set(new ComplexNumber(real * scalar, imaginary * scalar));
	}
	...
	*/
	 ...
	 * @return the complex number in x + yi format with the specified precision
	 */
	public String toString(int precision) {
		String re = String.format("%."+precision+"f", this.real);
		String im = "";
		if(this.imaginary < 0)
			im = String.format("%."+precision+"f", this.imaginary)+"i";
		else
			im = "+"+String.format("%."+precision+"f", this.imaginary)+"i";
		return re+im;
	}
	...
	/**
	 * 
	 * @param format the format of the complex number
	 * @return the complex number in the specified format
	 */
	public String toString(String format) {
		if(format.equals("xy"))
			return toString();
		else if(format.equals("rcis"))
			return format(RCIS);
		else if(format.equals("rcispi"))
			return format(RCISPI);
		else
			return toString();
	}
	 ...
	 * @return the complex number in x + yi format with the specified precision
	 */
	public String rcisFormat(int precision) {
		return String.format("%."+precision+"f", mod()) + " cis(" + String.format("%."+precision+"f", getArg()) + ")";
	}
	 ...
	 * @param format the format of the complex number
	 * @param precision the number of decimal places
	 * @return the complex number in the specified format with the specified precision
	 */
	public String toString(String format, int precision) {
		if(format.equals("xy"))
			return toString(precision);
		else if(format.equals("rcis"))
			return rcisFormat(precision);
		else if(format.equals("rcispi"))
			return String.format("%."+precision+"f", mod()) + " L(" + String.format("%."+precision+"f", getArgPI()) + "pi)";
		else
			return toString(precision);
	...
	*/
	...
	/**
	 * The norm of <code>ComplexNumber</code>
	...
	*/
	...
	/**
	 * The argument/phase of the current complex number in terms of PI.
	 * @return arg(z)/PI - the argument of current complex number
	 */
	public double getArgPI()
	{
	...
	*/
	public String format(int format_id) throws IllegalArgumentException
	{
		...
		else if (format_id == RCISPI) {
			out = mod()+" cis("+getArgPI()+")";
		}
		else
		{
